...
import {tRWA} from "./tRWA-multicollateral.sol";
import {IStrategy} from "../strategy/IStrategy.sol";
import {IHook} from "../hooks/IHook.sol";

/**
 * @title ManagedWithdrawRWA
 * @notice Extension of multi-collateral tRWA that implements manager-initiated withdrawals
 */
contract ManagedWithdrawRWA is tRWA {
     ...
     * @param asset_ Asset address (can be any token for accounting)
     * @param assetDecimals_ Decimals of the asset token
     * @param strategy_ Strategy address
     * @param sovaBTC_ SovaBTC address for redemptions
     */
    constructor(
        string memory name_,
        ...
        address strategy_,
        address sovaBTC_
    ) tRWA(name_, symbol_, asset_, assetDecimals_, strategy_, sovaBTC_) {}
    ...
    function batchRedeemShares(
        ...
        // Pull all SovaBTC from the strategy in a single transfer
        SafeTransferLib.safeTransferFrom(sovaBTC, strategy, address(this), totalAssets);
        ...
        for (uint256 i; i < len; ++i) {
            ...
            // Transfer SovaBTC to recipient
            SafeTransferLib.safeTransfer(sovaBTC, recipient, userAssets);
